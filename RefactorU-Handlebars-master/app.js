...
//access data OR use line 50
// app.get('/search', function(req, res){
//     var data = dummyData.JSON
//     req.query.
// 	res.send(data)
// });
...
//get my search term
app.get('/search', function(req,res){
	var dataToMine = JSON.parse(data)
	for(prop in dataToMine['programming']){
		if(prop === req.query.search){
			res.send(dataToMine['programming'][prop]['desc'])
		}
	}
	for(prop in dataToMine['search engines']){
		if(prop === req.query.search){
			res.send(dataToMine['search engines'][prop]['desc'])
		}
	}
});
